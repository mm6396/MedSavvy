...
const QueryBuilderEdit = () => {
	...
	useEffect(() => {
		...
		(async () => {
			try {
				...
				setModifyData(data.query);

				let loggedinUserRole = parseInt(localStorage.getItem('userRole'));
				form.setFieldsValue({
					roles: data.query.roles.filter(v => v !== loggedinUserRole),
					report_name: data.query.report_name,
					report_description: data.query.report_description,
					params: data.query.params,
					query: data.query.query,
          auto_send_query: data.query.auto_send_query,
  ...
  useEffect(() => {
	...
	const onFinish = async (value) => {
		...
		value.status = modifyData.status;
    value.type = value.type || null

		if (loc == '') {
			try {
				setLoading(true);
				const { data } = await FieldForceManagerAPI.patch(`/report/query-report/${id}/modify`, { value }, {
					headers: {
						Authorization: 'Bearer ' + localStorage.getItem("accessToken")
					}
				});
	...
	return (
		<div>
			<Spin spinning={loading}>
				<p style={{ color: "#004F9F", fontWeight: "bold", fontSize: "20px", margin: "0 0 30px 0", }}> Edit Query for Custom Report </p>
          ...
          <Col xs={{ span: 24, offset: 0 }} lg={{ span: 22, offset: 0 }}>
            ...
            <Form.Item name="auto_send_query">
              ...
              />
					...
					<br />
					<div style={{ textAlign: 'center' }}>
						<Button htmlType="submit" type='primary' shape="round"> Update Query {loading && <LoadingOutlined />}</Button>
					</div>
				</Form>
			</Spin>
		</div>
...
export default QueryBuilderEdit

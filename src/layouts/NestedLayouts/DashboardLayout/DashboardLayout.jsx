...
const DashboardLayout = () => {
    ...
    return (
        <Layout className="dashboard-div">
            <Affix offsetTop={90} style={phone === 'mobile' ? appStyle : null}  >
                <DashboardMenu />
            </Affix>
            <Layout style={phone === 'mobile' ? {padding: '0'} : { padding: '85px 24px 24px' }} >
                {/* <BreadCrumb /> */}
                <Content className="site-layout-background" style={{ padding: 24, margin: 20, minHeight: 700, background: 'white' }}>
                    <Switch>
                        {dashboardRoutes.map(v => v)}
                    </Switch>
                </Content>
